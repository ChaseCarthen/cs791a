...
int fromRawCreateMesh(vector<vector<float>>& input,float min, float max,GLuint& vbo_geometry,GLuint& elements)
{
    ...
    for(int i = 0; i < input.size(); i++)
    {
    	...
    	{   
        float val = ((float)input[i][j])/((float)max);
        //cout << input[i][j] << endl;
        //val += 1;
        if(val < 0)
        {
        ...
        else
        {
          ...
          vectors.push_back(temp);
    		...
    		{
    			indexs.push_back(i*input.size()+j);
    			indexs.push_back((i+1)*input.size()+j);
    			indexs.push_back((i+1)*input.size()+j+1);
    		}
    		// point, left, bottom
    		if(j-1 >= 0 && i+1 <input[i].size())
    		{
    			indexs.push_back(i*input.size()+j);
    			indexs.push_back((i)*input.size()+j-1);
    			indexs.push_back((i+1)*input.size()+j);
    		}
    	}
    }
